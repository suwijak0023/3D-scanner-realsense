...
class Scan(): 
    def __init__(self,width,height,framerate,autoexposureFrames,backDistance):
        ...
        self.pipe = rs.pipeline() # Define the process pipeline, Create a pipe
        self.config = rs.config() # Define configuration config
        self.config.enable_stream(rs.stream.color, self.width, self.height, rs.format.any, self.framerate) # To configure depth flow
        self.config.enable_stream(rs.stream.depth, self.width, self.height, rs.format.any, self.framerate) # To configure color flow
        ...
        # #STOCK
        # self.distance = 0.258 - self.backDistance   #ระยะห่าง
        # self.bbox = o3d.geometry.AxisAlignedBoundingBox((-0.13,-0.13,0),(0.13,0.13,0.2)) #ความกว้าง
        
        
        # self.distance = 0.428 - self.backDistance # subtract the difference from 30cm of .258 from 47cm (.47-(.3-.258))
        # self.bbox = o3d.geometry.AxisAlignedBoundingBox((-0.15,-0.15,0),(0.15,0.15,0.3)) # 30cm scan-able X/Y Area 30cm Z
        
        #
        self.distance = 0.315 - self.backDistance # subtract the difference from 30cm of .258 from 47cm (.47-(.3-.258))
        self.bbox = o3d.geometry.AxisAlignedBoundingBox((-0.15,-0.15,0),(0.15,0.15,30)) # 30cm scan-able X/Y Area 30cm Z
        
        # #
        # self.distance = 0.428 - self.backDistance 
        # self.bbox = o3d.geometry.AxisAlignedBoundingBox((-0.2,-0.2,0),(0.2,0.2,3)) 
                                                            # left       right


    def startPipeline(self):
        self.pipe.start(self.config)  # streaming The flow begins
...
# cap all
    ...
    def processFoto(self,angle):
        ...
        # self.rgbd_image = o3d.geometry.RGBDImage.create_from_color_and_depth(self.color_frame_open3d, self.depth_frame_open3d, convert_rgb_to_intensity=True)
        self.pcd = o3d.geometry.PointCloud.create_from_rgbd_image(self.rgbd_image, self.intrinsic)
        
        # self.pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1,max_nn=30)) # nn ความชัดภาพ+ความลึกในวัตถุ
        self.pcd.estimate_normals(search_param=o3d.geometry.KDTreeSearchParamHybrid(radius=0.1,max_nn=1))
        
        self.pcd.orient_normals_towards_camera_location(camera_location=np.array([0., 0., 0.]))
        # self.pcd.orient_normals_towards_camera_location(camera_location=np.array([100., 100., 100.])) # +++++
        
        self.getcameraLocation()
        self.rMatrix()
        self.pcd.rotate(self.R, (0, 0, 0))  #Stock
        # self.pcd.rotate(self.R, (5, 0, 0))
        #                           - , -
        self.pcd.translate((self.x,self.y,self.z))
        self.pcd = self.pcd.crop(self.bbox)
        #  self.pcd, self.ind = self.pcd.remove_statistical_outlier(nb_neighbors=100,std_ratio=2) #nb_meighbors เยอะนาน
    ...
    def getcameraLocation(self):
        ...
        self.x = np.sin(self.angle*self.dtr) * self.distance - np.cos(self.angle*self.dtr) * 0.0015  # width x -
        self.y = -np.cos(self.angle*self.dtr) * self.distance - np.sin(self.angle*self.dtr) * 0.035  # เเนวตรง l
        
        # stock ระยะความสูง z จากกลางหมุนไปยังกล้อง ถ้าเยอะใกล้
        # self.z = 0.165   # เห็นฐานบนจานหมุน    
        self.z = 0.265 # (Add 10cm to .165) เห็นฐานล่างจานหมุน  
        # self.z = 0.365 # เห็นพื้นจานหมุน  
        # self.z = 0.665
    ...
    def makeSTL(self,kpoints,stdRatio,depth,iterations):
        ...
        self.bbox1 = o3d.geometry.AxisAlignedBoundingBox((-0.15,-0.15,0),(0.15,0.15,0.01)) # 30cm scan-able X/Y Area
...
class App(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.title("3D-Scanner With Realsense")
...
class StartPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        # Add on Show realtime
        self.buttonStart = tk.Button(self.buttonFrame, text = 'Show Real Time', width = 25, command = self.controller.Realtime)
        self.buttonStart.grid(sticky="W",row = 6, column = 0, pady = 5, padx = 10)
        ...
        # zoom cut frame
        # self.load = Image.fromarray(np.zeros(shape=(480,848,3)), 'RGB')
        # self.left,self.upper,self.right,self.lower = 130,100,670,480
        
        # full screen
        self.load = Image.fromarray(np.zeros(shape=(480,848,3)), 'RGB')
        self.left,self.upper,self.right,self.lower = 130,20,670,480  #focus กล้อง+ขนาดเเอป
    ...
    # def showImagerealtime(self,iArray):
    #     self.load = Image.fromarray(iArray, 'RGB')
    #     self.render = ImageTk.PhotoImage(self.load.crop((self.left,self.upper,self.right,self.lower)))
    #     self.canvas.create_image(0,0, anchor='nw', image=self.render)    
    #     self.canvas.image = self.render
